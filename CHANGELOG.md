...
- Persist Hyprland stable window IDs so same-process address reuse cannot move a different browser window
- Reject partial process identity as proof that a saved address still belongs to the captured window
- Revalidate window identity immediately before address-based placement dispatches
...
- Avoid duplicate safety-recovery launches when an existing same-class window has an untrusted identity
- Refuse replacement-completion proof when the safety snapshot lacks window addresses

## [0.3.6] - 2026-08-27

### Fixed

- Recognize safely relaunched windows during interrupted replacement recovery
- Report missing Brave profile binaries as restore failures
- Keep recovery completion checks fail-closed for ambiguous browser identity
